...
for p in 'naive_B_cell' 'platelet' 'CD56_bright_natural_killer_cell_human' 'mature_NK_T_cell' 'CD14_positive_monocyte'; do
...
for d in $(ls -d $outdir/*/); do
    echo "python run_design_merge.py ${outdir}${d}/" | \
...
for d in $(ls -d $outdir/*/); do
    echo "python run_milo.py ${outdir}${d}/" | \
