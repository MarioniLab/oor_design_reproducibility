...
parser.add_argument("perturb_pop_remove",
                    type=str,
                    help="ID of removed population")
parser.add_argument("perturb_pop_shift",
                    type=str,
                    help="ID of shifted population")
...
perturb_pop = [args.perturb_pop_remove, args.perturb_pop_shift]
...
adata = simulate_query_reference(
    ...
    perturbation_type=['remove', 'shift']
)
assert check_dataset(adata)

# Save intermediate files
sim_id = f"qPBMC_{n_cells_subsample}cells_demo_mixed_{annotation_col}{clean_pop_name('-'.join(perturb_pop))}_queryBatchdataset_id{query_dataset}_seed{split_seed}"
