...
for m in scVI scArches; do
    ...
    d=ACR
    echo "python run_COVID_design.py ${d} ${m}" | \
            bsub -G teichlab -o logfile-COVID-%J.out -e logfile-COVID-%J.err -q gpu-normal -M150000 -R "select[mem>150000] rusage[mem=150000]" -gpu "mode=shared:j_exclusive=no:gmem=6000:num=1" 
    # done
done